	...
	*/
	...
	cls.prototype.$invokeHandling = function() {
		...
		if (cr != null) {
			...
			// this.$handlingdone = true; // Registrations done in handlers will start a deadloop - movig it before real invocation
			return r;
		}
	}
	cls.prototype.onBeforeOperationCompleted = function() {
		// TODO implement to do something additional before handling starts
	}
	cls.prototype.onAfterOperationCompleted = function() {
		// TODO implement to do something additional after handling ends
	}
	cls.prototype.onCompleteOperation = function () {
		this.onBeforeOperationCompleted();
		this.$invokeHandling();
		this.onAfterOperationCompleted();
	}
	...
	cls.prototype.set_completionroutine = function (v) {
		if (BaseObject.isCallback(v)) {
			...
			throw "Completion routine must be a valid callback (function, delegate, dispatcher etc.) or null, callbacks are either Function or objects supporting IInvokeWithArgsArray interface.";
		}
	}
}
