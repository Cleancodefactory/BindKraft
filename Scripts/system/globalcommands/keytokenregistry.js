System.CommandLibs.TokenStorage = function() {
    function _checkObject(obj) {
        if (typeof obj == "object" && obj != null) {
            ...
            if (obj.key.length == 0)
                return false;
            if (typeof obj.token == "string" && obj.token.length > 0 || typeof obj.servicename == "string" && obj.servicename.length > 0) {
                return true;
            }
        }
        return false;
                        ...
                        for (var k in result.data) {
                            if (result.data.hasOwnProperty(k)) {
                                if (_checkObject(result.data[k])) {
                                    if (!_registerToken(result.data[k]))
                                        some_err = true;
                                }
                            }
                        }
