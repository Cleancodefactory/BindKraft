(function() {
    var ITemplateSourceImpl = InterfaceImplementer("ITemplateSourceImpl");

    function DropDownControl2() {
        Control.apply(this, arguments);
        Messenger.Instance().subscribe("PageEvent",this.onHandlePageEvent);
    }    
    DropDownControl2.Inherit(Control, "DropDownControl2")
        .Implement(InterfaceImplementer("IAmbientDefaultsConsumerImpl"))
        .Implement(ICustomParameterizationStdImpl)
        .Implement(IItemTemplateConsumerImpl)
        .Implement(IItemTemplateSourceImpl, true, "single")
        .Implement(ITemplateSourceImpl, new Defaults("templateName", "bindkraft/control-dropdown"),"autofill")
        .Defaults({
            templateName: "bindkraft/control-dropdown",
            disabledCss: "disabled",
            enabledCss: null, // Usually not needed - the default form is enabled.
           
        });

    DropDownControl2.prototype.obliterate = function () {
        Messenger.Instance().unsubscribe("PageEvent",this.onHandlePageEvent);
        Base.prototype.obliterate.call(this);
    }
    DropDownControl2.prototype.onHandlePageEvent = new InitializeMethodDelegate("",function(msg) { //Event thrown when ESC or key up on every element on the workspace
