...
SvgY.prototype.isLoading = false;
...
SvgY.prototype.RequestSvg = function () {
    ...
    // if (!(svgPath in SvgY.svgStorage)) {
    //     SvgY.svgStorage[this.get_svgpath()] = null;
    ...
    // else {
    //     if (!this.isLoading) {
    //         let boundCallback = this.LoadSvg.bind(this);
    //         //setTimeout(function(){ boundCallback }, 1000);
    //         this.async(boundCallback).key(this.$__instanceId).after(200).execute();
    //         this.isLoading = true;
    //     }
    // }
}
...
SvgY.prototype.LoadSvg = function () {
    if (SvgY.svgStorage.hasOwnProperty(this.get_svgpath()) && SvgY.svgStorage[this.get_svgpath()] != null) {
        ...
        this.discardAsync(this.$__instanceId);
        return;
    }
